		...
		if (loading) {
			...
			if (typeof (this.state.contents) !== 'undefined' && Array.isArray (this.state.contents)) {
				for (let index in this.state.contents) {
					...
					contents.push (<div key={rowData.reactId} className={`tch-grid-row tch-grid-action-row ${typeof (rowData.selected) && rowData.selected ? ' active' : ''}`} onClick={this.FileAction.bind (this)} data-reactid={rowData.reactId} data-action="row">
						{row}
					</div>);
				}
			}
